...
public record C2SSetBloodSuckPacket(int entityId, boolean isBloodSuck) implements CustomPayload {
  ...
  public static void receive(C2SSetBloodSuckPacket payload, NetworkManager.PacketContext context) {
    context.queue(
        () -> applyBloodSuckServer(context.getPlayer(), payload.entityId(), payload.isBloodSuck()));
  }
