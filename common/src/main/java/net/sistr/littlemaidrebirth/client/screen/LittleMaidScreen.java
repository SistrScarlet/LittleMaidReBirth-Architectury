...
public class LittleMaidScreen extends HandledScreen<LittleMaidScreenHandler> {
    ...
    this.addDrawableChild(
        new IconButtonWidget(
          ...
          protected void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            setTooltip(
                Tooltip.of(LittleMaidScreen.this.owner.isBloodSuck() ? isBloodSuck : toBloodSuck));
          }
        });
    layer = -1;
    this.addDrawableChild(
  ...
  protected void drawForeground(DrawContext context, int mouseX, int mouseY) {
    ...
    String insideSkirt =
        Text.translatable("entity.littlemaidrebirth.little_maid_mob.InsideSkirt").getString();
    context.drawText(
        textRenderer, insideSkirt, 168 - textRenderer.getWidth(insideSkirt), 65, 0x404040, false);
    float left = (width - backgroundWidth) / 2F;
    float top = (height - backgroundHeight) / 2F;
  ...
  protected void drawHealth(DrawContext context, int mouseX, int mouseY) {
    ...
    if (left + 98 <= mouseX
        ...
        && mouseY < top + 7 + 2 * 9) {
      String healthStr =
          MathHelper.ceil(owner.getHealth()) + " / " + MathHelper.ceil(owner.getMaxHealth());
      context.drawText(
          textRenderer,
          healthStr,
  ...
  public static class IconButtonWidget extends ButtonWidget {
    ...
    protected void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
      super.renderButton(context, mouseX, mouseY, delta);
      context.drawItem(
          getIconItem(), this.getX() - 8 + this.width / 2, this.getY() - 8 + this.height / 2);
    }
  }
}
