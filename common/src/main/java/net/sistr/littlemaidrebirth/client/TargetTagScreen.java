...
public class TargetTagScreen extends Screen {
    ...
    protected void init() {
        ...
        List<TargetTagGUIElement> elements = targetTags.entrySet().stream()
                .map(entry ->
                        new TargetTagGUIElement(this.client.textRenderer, entry.getKey(), entry.getValue()))
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        return targetTagGui.mouseScrolled(mouseX, mouseY, amount);
    }
    ...
    public <T extends Entity & TargetTagManager> void send(Map<TargetIdentifier, Set<TargetingSystem.TargetTag>> updatedTargetTags) {
    ...
    public static class TargetTagGUIElement extends GUIElement implements ListGUIElement {
        ...
        public TargetTagGUIElement(TextRenderer textRenderer,
            ...
            this.buttons.add(new LittleMaidScreen.IconButtonWidget(0, 0, this.attackState.icon,
                    Text.translatable("gui.littlemaidrebirth.target_tag.tags." + this.attackState.translationKey),
                    (b) -> this.attackState = this.attackState.next()));

            // 武器ボタン
            this.buttons.add(new LittleMaidScreen.IconButtonWidget(0, 0, this.weaponState.icon,
                    Text.translatable("gui.littlemaidrebirth.target_tag.tags." + this.weaponState.translationKey),
            ...
            this.buttons.add(new LittleMaidScreen.IconButtonWidget(0, 0, this.approachState.icon,
                    Text.translatable("gui.littlemaidrebirth.target_tag.tags." + this.approachState.translationKey),
        ...
        public void render(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            // エンティティタイプ名を表示
            var entityTypeName = Text.translatable(this.targetIdentifier.getEntityType().getTranslationKey());
            int textWidth = textRenderer.getWidth(entityTypeName);
            if (textWidth <= this.width) {
                context.drawText(textRenderer, entityTypeName,
                ...
                // クリップ領域を設定してテキストを描画
                context.enableScissor(this.x, this.y, this.x + displayWidth, this.y + textRenderer.fontHeight);
                context.drawText(textRenderer, entityTypeNameStr,
            ...
            attackButton.setTooltip(Tooltip.of(Text.translatable("gui.littlemaidrebirth.target_tag.tags." + attackState.translationKey)));
            ...
            weaponButton.setTooltip(Tooltip.of(Text.translatable("gui.littlemaidrebirth.target_tag.tags." + weaponState.translationKey)));
            ...
            approachButton.setTooltip(Tooltip.of(Text.translatable("gui.littlemaidrebirth.target_tag.tags." + approachState.translationKey)));
