  ...
  public static class LMInfoGUIElement extends GUIElement implements ListGUIElement {
    ...
    public LMInfoGUIElement(TextRenderer textRenderer, MaidManager.LMInfo lmInfo) {
      ...
      this.callWaitButton =
          new ButtonWidget.Builder(
                  ...
                  onPress -> {
                    lmInfo
                        .getEntityClient(MinecraftClient.getInstance().world)
          ...
          .map(
              entity -> {
                ...
                double squaredDistance =
                    client.player.squaredDistanceTo(entity.getX(), entity.getY(), entity.getZ());
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      if (lmInfo.status() != MaidManager.Status.SOUL_WITHIN) {
        ...
        statusText =
            statusText.append(Text.literal(" / ").formatted(Formatting.GRAY)).append(loadedText);
      }

      context.drawText(
          textRenderer, statusText, this.x, this.y + textRenderer.fontHeight, 0xFFCCCCCC, true);
      ...
      if (!worldId.isEmpty()) {
        ...
        if (worldId.equals(client.world.getRegistryKey().getValue().toString())) {
          ...
          Text distanceText =
              Text.literal(String.format(" (%.0fm)", distance)).formatted(distanceColor);
                      ...
                      .ifPresent(
                          modeName ->
                              context.drawText(
                                  ...
                                  modeName,
                                  this.x,
                                  this.y + textRenderer.fontHeight * 4,
                                  0xFFFFFFFF,
                                  true));
                }
