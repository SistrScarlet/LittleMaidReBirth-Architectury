...
public class MaidManagerScreen extends Screen {
    ...
    protected void init() {
        ...
        this.lmInfoGui = FilterableListGUI.<LMInfoGUIElement>builder()
                .position(MathHelper.floor((this.width - totalWidth) / 2f),
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        return lmInfoGui.mouseScrolled(mouseX, mouseY, amount);
    }
    ...
    private static Comparator<LMInfoGUIElement> createSortComparator(String currentWorldId) {
        return Comparator
                ...
                .thenComparing(e -> {
                    ...
                    return e.getLMInfo().getEntityClient(client.world)
                            .map(entity -> client.player.squaredDistanceTo(
    ...
    public static class LMInfoGUIElement extends GUIElement implements ListGUIElement {
        ...
        public LMInfoGUIElement(TextRenderer textRenderer, MaidManager.LMInfo lmInfo) {
            ...
            this.callWaitButton = new ButtonWidget.Builder(
                    Text.literal("call"),
                    onPress -> {
                        lmInfo.getEntityClient(MinecraftClient.getInstance().world)
                                ...
                                .ifPresent(e ->
                                        C2SCallWaitPacket.sendC2SPacket(e,
        ...
        private boolean canInteractWithMaid() {
            ...
            if (worldId.isEmpty() || !worldId.equals(client.world.getRegistryKey().getValue().toString())) {
                return false;
            }
        ...
        public void render(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            context.fill(this.x, this.y,
                    this.x + this.width - textRenderer.fontHeight, this.y + textRenderer.fontHeight,
                    0xFF000000);
            String name = lmInfo.getEntityClient(client.world)
                    .map(entity -> entity.getName().getString())
                    .orElse(lmInfo.name());
            drawScrollingText(context, textRenderer, name, this.x, this.y,
                    this.width - textRenderer.fontHeight, 0xFFFFFFFF, false);
            ...
            context.drawText(textRenderer, statusText,
                    this.x, this.y + textRenderer.fontHeight, 0xFFCCCCCC, true);
            ...
            if (!worldId.isEmpty()) {
                drawScrollingText(context, textRenderer, worldId, this.x,
                        ...
                        this.width - textRenderer.fontHeight * 3, 0xFFAAAAAA, true);
            }

            // 4. XYZ座標と距離を表示（worldIdが空でない場合のみ）
            if (!worldId.isEmpty()) {
                BlockPos pos = lmInfo.getEntityClient(client.world)
                        .map(entity -> entity.getBlockPos())
                        .orElse(lmInfo.getLastPos());

                var coordText = Text.literal(String.format("XYZ: %d, %d, %d",
                ...
                if (worldId.equals(client.world.getRegistryKey().getValue().toString())) {
                    double squaredDistance = client.player
                            .squaredDistanceTo(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5);
                    ...
                    .ifPresent(e -> {
                        ...
                        if (e instanceof LittleMaidEntity littleMaid) {
                            littleMaid.getModeName()
                                    .ifPresent(modeName -> context.drawText(textRenderer, modeName,
                                            this.x, this.y + textRenderer.fontHeight * 4,
                                            0xFFFFFFFF, true));
                        }
                        ...
                        int entitySize = 20;
                        InventoryScreen.drawEntity(context, entityX, entityY, entitySize, 20, 0, e);
                    });
         ...
         */
        private void drawScrollingText(DrawContext context, TextRenderer textRenderer, String text,
                                       int x, int y, int availableWidth, int color, boolean shadow) {
            drawScrollingText(context, textRenderer, Text.of(text), x, y, availableWidth, color, shadow);
        }

        private void drawScrollingText(DrawContext context, TextRenderer textRenderer, Text text,
                                       int x, int y, int availableWidth, int color, boolean shadow) {
