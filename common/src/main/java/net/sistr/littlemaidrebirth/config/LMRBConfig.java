...
public class LMRBConfig implements ConfigData {
  ...
  public static class Movement {
    ...
    @ConfigEntry.Gui.Tooltip public float freedomRange = 16.0f;
    ...
    @ConfigEntry.Gui.Tooltip public float followStartDistance = 6.0f;
    ...
    @ConfigEntry.Gui.Tooltip public float emergencyTeleportStartDistance = 6.0f;
  ...
  public static class Target {
    ...
    @ConfigEntry.Gui.Tooltip public int dangerousAvoidDistance = 8; // 危険敵回避距離（クリーパー等から距離を取る）

    // 分散ターゲティング設定（集中攻撃を防ぐ）
    @ConfigEntry.Gui.Tooltip public double distributionRatio = 0.5; // 分散比率（メイドさん数の50%が同じ敵を攻撃）
    @ConfigEntry.Gui.Tooltip public int maxAttackersPerTarget = 2; // 1体あたり最大攻撃者数（集中攻撃防止）

    // 体力関連設定
    @ConfigEntry.Gui.Tooltip public float injuredThreshold = 0.5f; // 負傷判定闾値（体力50%以下で負傷扱い）
    @ConfigEntry.Gui.Tooltip public int attackedByValidTicks = 200; // 攻撃判定有効時間（10秒間、200tick）
  }
