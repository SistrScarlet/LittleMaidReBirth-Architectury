  ...
  public boolean shouldExecute() {
    return (!this.mob.getProjectileType(this.mob.getMainHandStack()).isEmpty()
            || EnchantmentHelper.getLevel(
                    LMEnchantmentUtil.entry(this.mob.getWorld(), Enchantments.INFINITY),
                    this.mob.getMainHandStack())
                > 0)
        && super.shouldExecute();
  }
    ...
    if (itemStack.getItem() instanceof BowItem) {
      ...
      if (!CrossbowItem.isCharged(itemStack)) {
        ...
        if (result.isPresent()) {
          ...
          this.mob.shootAt(target, 1.0f);
          itemStack.set(
              DataComponentTypes.CHARGED_PROJECTILES, ChargedProjectilesComponent.DEFAULT);
          this.mob.play(LMSounds.SHOOT);
          this.mob.swingHand(Hand.MAIN_HAND);
        }
      }
