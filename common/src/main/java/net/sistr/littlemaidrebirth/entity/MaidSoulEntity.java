...
public class MaidSoulEntity extends Entity {
    ...
    @Override
    public void tick() {
        if (!this.getWorld().isClient() && !this.maidManagerRegistered && this.maidSoul != null) {
            this.maidSoul
                    .getOwnerUUID()
        ...
        float y =
                MathHelper.sin(MathHelper.PI * 2 * ((float) (this.waveProgress % loop) / loop))
                        * (waveHeight / 2);
        ...
        var world = getWorld();
        world.addParticle(
                particle, this.getX() + x, this.getY() + y + yOffset, this.getZ() + z, 0, 0, 0);
        world.addParticle(
                particle, this.getX() - x, this.getY() + y + yOffset, this.getZ() - z, 0, 0, 0);
        world.addParticle(
                particle, this.getX() - x, this.getY() - y + yOffset, this.getZ() - z, 0, 0, 0);
