...
public class HasModeImpl implements HasMode {
  ...
  public HasModeImpl(
      LivingEntity owner, HasInventory hasInventory, Set<Mode> modes, Consumer<Mode> onModeChange) {
    this.owner = owner;
  ...
  protected void updateMatchList() {
    ...
    this.modes.stream()
        ...
        .sorted(
            Comparator.<Tuple<Mode, Tuple<ItemMatcher.Priority, ItemMatcher>>>comparingInt(
                    tuple -> tuple.getB().getA().get())
                .reversed())
    ...
    if (nbt.contains("ModeType") && nbt.contains("ModeData")) {
      ...
      if (modeID != null) {
        ...
        ModeManager.INSTANCE
            ...
            .flatMap(
                modeType ->
                    modes.stream().filter(mode -> mode.getModeType() == modeType).findFirst())
            .ifPresent(
                mode -> {
