...
public class HealerMode extends Mode {
  ...
  public boolean isFood(ItemStack stack) {
    ...
    return food != null
        && food.effects().stream()
            ...
            .noneMatch(
                p ->
                    p.effect().getEffectType().value().getCategory()
  ...
  public boolean isBeneficialPotion(LivingEntity owner, ItemStack stack, boolean fullHealth) {
    ...
    if (effects.stream()
        .anyMatch(
            e -> e.getEffectType().value().getCategory() != StatusEffectCategory.BENEFICIAL)) {
      return false;
    }
    ...
    return effects.stream()
        ...
        .anyMatch(
            e ->
                owner.getStatusEffects().isEmpty()
                    || owner.getStatusEffects().stream()
                        // いずれか一つでもご主人が持ってたらダメ
                        .noneMatch(
                            oE ->
                                oE.getEffectType() == e.getEffectType()
  ...
  public void tick() {
    ...
    if (foodIndex != -1) {
      ...
      stack = owner.tryEatFood(owner.getWorld(), stack);
      if (stack.isEmpty()) {
