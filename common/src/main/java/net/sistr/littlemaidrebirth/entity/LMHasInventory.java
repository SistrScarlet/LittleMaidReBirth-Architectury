...
public class LMHasInventory implements HasInventory {
  ...
  @Override
  public void writeInventory(NbtCompound nbt, RegistryWrapper.WrapperLookup lookup) {
    nbt.put("Inventory", this.writeNbt(new NbtList(), lookup));
    nbt.putByte("workItemSlotSize", (byte) this.workItemSlotSize);
  }

  @Override
  public void readInventory(NbtCompound nbt, RegistryWrapper.WrapperLookup lookup) {
    int maidVersion = nbt.getByte("maidVersion") & 255;
    if (maidVersion == 0) {
      this.readNbtOld(nbt.getList("Inventory", 10), lookup);
    } else {
      this.readNbt(nbt.getList("Inventory", 10), lookup);
    }
  ...
  public NbtList writeNbt(NbtList nbtList, RegistryWrapper.WrapperLookup lookup) {
    ...
    for (i = 0; i < 18; ++i) {
      ...
      if (!stack.isEmpty()) {
        ...
        nbtList.add(stack.encode(lookup, nbt));
      }
    }
  ...
  public void readNbt(NbtList nbtList, RegistryWrapper.WrapperLookup lookup) {
    ...
    for (int i = 0; i < nbtList.size(); ++i) {
      ...
      ItemStack stack = ItemStack.fromNbtOrEmpty(lookup, nbtCompound);
      if (!stack.isEmpty()) {
        if (j < 18) {
  ...
  public void readNbtOld(NbtList nbtList, RegistryWrapper.WrapperLookup lookup) {
    this.inventory.clear();
