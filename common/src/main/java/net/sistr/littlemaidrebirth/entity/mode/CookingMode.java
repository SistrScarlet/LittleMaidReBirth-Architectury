...
public class CookingMode extends Mode {
  ...
  private static final Object2ObjectOpenHashMap<BlockPos, LittleMaidEntity> USED_FURNACE_MAP =
  ...
  public boolean canCookingFurnace(AbstractFurnaceBlockEntity tile) {
    RecipeType<? extends AbstractCookingRecipe> recipeType =
        ((AbstractFurnaceAccessor) tile).getRecipeType_LM();
    for (int slot : tile.getAvailableSlots(Direction.UP)) {
      ItemStack stack = tile.getStack(slot);
      if (!stack.isEmpty()) continue;
      // 手持ちに焼けるアイテムがあればtrue
      if (getAnyCookableItem(recipeType, cookable -> tile.canInsert(slot, cookable, Direction.UP))
          .isPresent()) {
        return true;
      }
    }
  ...
  public Optional<ItemStack> getAnyCookableItem(
      RecipeType<? extends AbstractCookingRecipe> recipeType, Predicate<ItemStack> predicate) {
    Inventory inventory = this.mob.getInventory();
    for (int i = 0; i < inventory.size(); ++i) {
  ...
  public Optional<? extends AbstractCookingRecipe> getRecipe(
      ItemStack stack, RecipeType<? extends AbstractCookingRecipe> recipeType) {
    return mob.getWorld()
        .getRecipeManager()
        .getFirstMatch(recipeType, new SimpleInventory(stack), mob.getWorld());
  }
  ...
  public void tick() {
    ...
    this.mob
        .getLookControl()
        .lookAt(furnacePos.getX() + 0.5, furnacePos.getY() + 0.5, furnacePos.getZ() + 0.5);
    ...
    RecipeType<? extends AbstractCookingRecipe> recipeType =
        ((AbstractFurnaceAccessor) furnace).getRecipeType_LM();

    playSoundCool--;

    // 焼けるものがあれば突っ込む
    getCookable(recipeType)
        .ifPresent(cookableIndex -> tryInsertCookable(furnace, inventory, cookableIndex));
    // 燃料があれば突っ込む
  ...
  private void tryInsertCookable(
      AbstractFurnaceBlockEntity furnace, Inventory inventory, int cookableIndex) {
  ...
  private void tryInsertFuel(
      AbstractFurnaceBlockEntity furnace, Inventory inventory, int fuelIndex) {
  ...
  public void writeModeData(NbtCompound nbt) {
    if (furnacePos != null) nbt.put("FurnacePos", NbtHelper.fromBlockPos(furnacePos));
  }
