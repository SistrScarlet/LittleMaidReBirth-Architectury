...
public class BrewingWorkStrategy implements WorkStrategy<BrewingStandBlockEntity> {
  ...
  public boolean hasRequiredItems(Inventory inventory, World world) {
    return getIngredientSlot(inventory, world.getBrewingRecipeRegistry()).isPresent()
  ...
  public boolean isUsableTarget(BrewingStandBlockEntity stand, Inventory inventory, World world) {
    return isEmpty(stand)
        && hasBrewableRecipeInInventory(inventory, world.getBrewingRecipeRegistry());
  }
  ...
  public boolean shouldContinueWork(
    ...
    return hasBrewableRecipeInInventory(inventory, world.getBrewingRecipeRegistry());
  }
  ...
  public void doWork(
    ...
    var registry = world.getBrewingRecipeRegistry();
    boolean hasPotionInStand = hasPotionsInStand(stand);

    if (hasPotionInStand) {
      // 醸造台にポーションがある: 連続醸造または回収
      tryExtractUnusableIngredient(stand, inventory, actions, registry);

      if (stand.getStack(INGREDIENT_SLOT).isEmpty()) {
        OptionalInt nextIngredient = getMatchingIngredient(stand, inventory, registry);
      ...
      tryInsertFuel(stand, inventory, actions);
      tryInsertPotionBottles(stand, inventory, actions, registry);

      if (hasPotionsInStand(stand)) {
        OptionalInt matchingIngredient = getMatchingIngredient(stand, inventory, registry);
  ...
  private OptionalInt getIngredientSlot(Inventory inventory, BrewingRecipeRegistry registry) {
    for (int i = 0; i < inventory.size(); ++i) {
      ItemStack stack = inventory.getStack(i);
      if (!stack.isEmpty() && registry.isValidIngredient(stack)) {
  ...
  private boolean hasBrewableRecipeInInventory(
      Inventory inventory, BrewingRecipeRegistry registry) {
    for (int i = 0; i < inventory.size(); ++i) {
      ...
      for (int j = 0; j < inventory.size(); ++j) {
        ...
        if (!potion.isEmpty() && isPotionItem(potion) && registry.hasRecipe(potion, ingredient)) {
          return true;
        }
      }
    }
  ...
  private OptionalInt getMatchingIngredient(
      BrewingStandBlockEntity stand, Inventory inventory, BrewingRecipeRegistry registry) {
    for (int i = 0; i < inventory.size(); ++i) {
      ItemStack invStack = inventory.getStack(i);
      if (invStack.isEmpty() || !registry.isValidIngredient(invStack)) {
        continue;
      }
      for (int slot = POTION_SLOT_START; slot <= POTION_SLOT_END; slot++) {
        ItemStack potionStack = stand.getStack(slot);
        if (!potionStack.isEmpty() && registry.hasRecipe(potionStack, invStack)) {
          return OptionalInt.of(i);
        }
      }
    }
  ...
  private void tryInsertPotionBottles(
    ...
    for (int slot = POTION_SLOT_START; slot <= POTION_SLOT_END; slot++) {
      ...
      for (int i = 0; i < inventory.size(); ++i) {
        ...
        if (!hasMatchingIngredientFor(potionStack, inventory, registry)) {
          continue;
        }
  ...
  private boolean hasMatchingIngredientFor(
      ItemStack potion, Inventory inventory, BrewingRecipeRegistry registry) {
    for (int i = 0; i < inventory.size(); ++i) {
      ItemStack stack = inventory.getStack(i);
      if (!stack.isEmpty()
          && registry.isValidIngredient(stack)
          && registry.hasRecipe(potion, stack)) {
        return true;
      }
    }
    return false;
  ...
  private void tryExtractUnusableIngredient(
    ...
    for (int slot = POTION_SLOT_START; slot <= POTION_SLOT_END; slot++) {
      ItemStack potionStack = stand.getStack(slot);
      if (!potionStack.isEmpty() && registry.hasRecipe(potionStack, ingredient)) {
        return;
      }
    }
    tryTransferToInventory(stand, INGREDIENT_SLOT, inventory, actions);
