...
public class Modes {
        ...
        .addItemMatcher(
            stack ->
                stack.getItem() instanceof BlockItem
                    && 9
                        < ((BlockItem) stack.getItem()).getBlock().getDefaultState().getLuminance(),
  ...
  public static ModeType.Builder<HealerMode> buildHealerMode() {
    return ModeType.<HealerMode>builder((type, maid) -> new HealerMode(type, "Healer", maid))
        .addItemMatcher(
            stack -> stack.contains(DataComponentTypes.FOOD), ItemMatcher.Priority.LOWER)
        .addItemMatcher(
            stack -> {
              PotionContentsComponent potion = stack.get(DataComponentTypes.POTION_CONTENTS);
              return potion != null && potion.hasEffects();
            },
            ItemMatcher.Priority.LOWER)
  ...
  public static ModeType.Builder<BlockWorkMode> buildPharmacistMode() {
    return ModeType.<BlockWorkMode>builder(
        ...
        .addItemMatcher(
            stack -> {
              PotionContentsComponent potion = stack.get(DataComponentTypes.POTION_CONTENTS);
              return potion != null && potion.potion().isPresent() && !potion.hasEffects();
            },
            ItemMatcher.Priority.LOWER)
        .addItemMatcher(
