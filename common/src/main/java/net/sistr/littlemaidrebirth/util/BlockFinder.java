...
public class BlockFinder {
  ...
  public static void seedFill(
      BlockPos seed,
      final int maxCount,
      Predicate<BlockPos> fillable,
      Consumer<BlockPos> filler,
  ...
  // 多分動かん
  public static Optional<BlockPos> findTarget(
      BlockPos centerPos, int range, Predicate<BlockPos> target, Predicate<BlockPos> linkable) {
    ...
    for (int i = 0; i < range; i++) {
      for (BlockPos checkedPos : checked) {
        for (Direction direction : Direction.values()) {
          int num =
              checkedPos.getComponentAlongAxis(direction.getAxis())
                  - centerPos.getComponentAlongAxis(direction.getAxis());
          // 0の場合は両方向にやる
          if (0 <= num) {
            BlockPos checkPos = checkedPos.offset(direction);
            if (target.test(checkPos)) return Optional.of(checkPos);
            if (linkable.test(checkPos)) nowChecked.add(checkPos);
          }
          if (num <= 0) {
  ...
  public static Optional<BlockPos> findHorizonPos(
      World world, BlockPos centerPos, int layer, int horizon, Predicate<BlockPos> target) {
    // 垂直方向に5ブロック調査
    for (int l = 0; l < layer; l++) {
      BlockPos checkPos =
          centerPos.offset(layer % 2 == 0 ? Direction.UP : Direction.DOWN, layer >> 1);
  ...
  public static Optional<BlockPos> findLayer(
      World world, BlockPos centerPos, int horizon, Predicate<BlockPos> target) {
  ...
  public static Optional<BlockPos> findHorizon(
      World world,
      Set<BlockPos> prevSearched,
      Set<BlockPos> allSearched,
  ...
  /**
   * 特定ブロックを探索する。 近い地点から探索していく。 微妙かもしらん…
   *
   * @param seed 開始地点
   * @param target 探索対象
   ...
   */
  public static Optional<BlockPos> searchTargetBlock(
      BlockPos seed,
      Predicate<BlockPos> target,
      Predicate<BlockPos> linkable,
